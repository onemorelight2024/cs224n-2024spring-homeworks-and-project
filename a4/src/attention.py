...
def precompute_rotary_emb(dim, max_positions):
    ...
    ### YOUR CODE HERE ###
    #利用矩阵外积实现
    t_arr=torch.arange(0,max_positions,dtype=torch.float32)
    i_arr=torch.arange(0,dim/2,dtype=torch.float32)
    i_arr=10000**(-2*i_arr/dim)#没有减一
    
    freq=torch.outer(t_arr,i_arr)
    #(max_positions, dim/2),pos在前
    
    cos_vals = torch.cos(freq)
    sin_vals = torch.sin(freq)
    
    cos_vals_unsqueezed = cos_vals.unsqueeze(-1)
    sin_vals_unsqueezed = sin_vals.unsqueeze(-1) 
    rope_cache = torch.cat([cos_vals_unsqueezed, sin_vals_unsqueezed], dim=-1)
...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    #pytorch里token是行向量
    #截取所需的长度->x分成二维向量、复数化->rope_cache复数化->相乘->转换形状
    batch_size, num_head,seq_len, dim = x.shape
    rope_cache_truncated = rope_cache[:seq_len,:, :]
    
    xx = x.view(batch_size, num_head,seq_len,dim // 2, 2)
    
    rope_cache_truncated=rope_cache_truncated.unsqueeze(0).unsqueeze(0)
    temp=torch.view_as_complex(rope_cache_truncated)*torch.view_as_complex(xx)
    rotated_x=torch.view_as_real(temp)
    rotated_x=rotated_x.view(batch_size,num_head, seq_len,dim)
    return rotated_x
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.rope:
            ...
            self.register_buffer("rope_cache", rope_cache)#缓存，随着模型一起保存(self.rope_cache会随着磁盘消失)
    ...
    def forward(self, x):
        #注意其中的转置操作!要把num_head放到第二个(第一个是batch_size)
        B, T, C = x.size()
