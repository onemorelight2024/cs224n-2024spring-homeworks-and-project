...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document1 = self.data[idx].split('\t')
        document=document1[0]     
        length=random.randint(4,int(self.block_size*7/8))      
        trun_document=document[:length]       
        masked_len=random.randint(int(length*0.2),int(length*0.3))
        random_start=random.randint(0,max(0,length-masked_len))
        prefix=trun_document[0:random_start]
        masked_con=trun_document[random_start:random_start+masked_len]
        suffix=trun_document[random_start+masked_len:]#-1是最后一个字符！！！左闭右开会漏掉！！！所以要写成[num:]
        masked_string_without_padding=prefix+self.MASK_CHAR+suffix+self.MASK_CHAR+masked_con
        #例子与注释不一致
        masked_string=masked_string_without_padding+self.PAD_CHAR*(self.block_size + 1-len(masked_string_without_padding))
        inputt=masked_string[:-1]
        outputt=masked_string[1:]
        x = torch.tensor([self.stoi[c] for c in inputt], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in outputt], dtype=torch.long)
        return (x,y)
        ### END YOUR CODE ###
