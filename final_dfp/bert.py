...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    #softmax之前要进行masked，把padding都换成负无穷
    head_dim=query.shape[-1]
    att_scores=torch.matmul(query,key.transpose(-2,-1))/math.sqrt(head_dim)#bmm只能处理三维向量
    att_scores_masked=att_scores+attention_mask
    att_fin=torch.softmax(att_scores_masked,-1)#softmax要指定维度
    att_fin=self.dropout(att_fin)  # attention和forword一样需要dropout！
    weighted_values=torch.matmul(att_fin,value)
    
    temp=weighted_values.permute(0,2,1,3).contiguous()#用permute而非view
    a,b,c,d=temp.shape
    final=temp.view(a,b,c*d)
    return final
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    # 先线性层-->dropout->然后add->norm
    # dropout是防止过拟合！所以要丢弃新学的特征！
    #这个残差连接是最最最开始的input
    output1=dense_layer(output)
    output2=dropout(output1)
    output3=input+output2
    output4=ln_layer(output3)
    return output4
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    ln_after_attention = self.add_norm(
        input=hidden_states,
    ...
    intermediate_output = self.interm_af(self.interm_dense(ln_after_attention))

    final_output = self.add_norm(
        input=ln_after_attention,
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    outputs=self.embed_dropout(self.embed_layer_norm(inputs_embeds+pos+tk_type_embeds))
    return outputs
  ...
  def forward_with_embed(self,embedding_output,attention_mask):
    # Feed to a transformer (a stack of BertLayers).
    sequence_output = self.encode(embedding_output, attention_mask=attention_mask)
