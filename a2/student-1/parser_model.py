...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.n_features = n_features#貌似没用
        ...
        '''为什么是这个？因为输入的是[1,embed_size],要把它映射到hidden_dim上。最后输出的num_classes，如情感分类任务则为2。'''
    ...
    def embedding_lookup(self, w):
        ...
        x = None
        '''不要使用loops'''
        x=self.embeddings[w]#(batch_size,n_feature,embed_dim)
    ...
    def forward(self, w):
        ...
        logits = None
        w=self.embeddings[w]#feature->embedd
        w=w.view(w.shape[0],-1)
        h=F.relu(torch.matmul(w,self.embed_to_hidden_weight)+self.embed_to_hidden_bias)#relu没有官方的直接函数
        h=self.dropout(h)
