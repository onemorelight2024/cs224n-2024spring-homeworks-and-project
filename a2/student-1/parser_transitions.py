...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        '''要展开啊！！！！！！！！！！！！！！如果buffer=[sentence]就是双重列表了'''
        ...
        ###             tuples where each tuple is of the form (head, dependent).哈哈哈哈哈哈哈哈哈我没看到
...
def minibatch_parse(sentences, model, batch_size):
    ...
    parital_parses=[PartialParse(sentence) for sentence in sentences] 
    unfinished_parses=parital_parses[:]
    while unfinished_parses:#并非not
        minibatch=unfinished_parses[:batch_size]
        trans=model.predict(minibatch)
        for index,tran in enumerate(trans):
            minibatch[index].parse_step(tran)
            if (len(minibatch[index].stack)==1 and not minibatch[index].buffer):
                unfinished_parses.remove(minibatch[index])
    dependencies=[partial.dependencies for partial in parital_parses]#注意这里千万不能'不按顺序返回'c
